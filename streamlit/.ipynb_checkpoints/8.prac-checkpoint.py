...
option = st.selectbox('하차역 선택 (2.df_off)',     .    )

# .loc 함수를 사용하여 선택한 역의 데이터를 선별하고
# 새로운 데이터 프레임에 저장
df_sta = df_off.loc[df_off['역명'] ==       ]
...
chart = alt.Chart(             ).mark_bar().encode(
         x=            ).properties(width=650, height=350)
text = alt.Chart(              ).mark_text(dx=0, dy=-10, color='black').encode(
         x=            , text=alt.Text('인원수:Q', format=',.0f') ) 
