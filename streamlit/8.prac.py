...
# 새로운 데이터프레임에 저장 & checkbox를 선택하면 데이터프레임이 나타남
df_line_groupby = df_line_melted.groupby(by=['호선', '시간'], as_index=False)['인원수'].sum()
...
# 새로운 데이터 프레임-에 저장 & 확인
df_sta_groupby = df_sta_melted.groupby(by='시간', as_index=False)['인원수'].sum()
st.write(option, '집계 데이터 (df_sta_groupby)', df_sta_groupby)

# altair mark_bar  chart + text 그리기
# 데이터프레임- df_sta_groupby,  x-'시간',  y-'인원수'
chart = alt.Chart(df_sta_groupby).mark_bar().encode(x='시간', y='인원수').properties(width=650, height=350)
text = alt.Chart(df_sta_groupby).mark_text(dx=0, dy=-10, color='black').encode(x='시간', y='인원수', text=alt.Text('인원수:Q', format=',.0f') ) 
# format=',.0f' : 천 단위 구분기호+소수점 이하 0

st.altair_chart(chart+text, use_container_width=True)


# 파일실행: File > New > Terminal(anaconda prompt) → python -m streamlit run streamlit\8.prac.py